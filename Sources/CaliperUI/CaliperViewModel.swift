...
public final class CaliperViewModel: ObservableObject {
    ...
    public func snapEndpoints() {
        ...
        if let s = engine.nearestSnap(to: startX, in: snapLines.verticalX) { startX = s }
        if let e = engine.nearestSnap(to: endX, in: snapLines.verticalX) { endX = e }
        if let s = engine.nearestSnap(to: startY, in: snapLines.horizontalY) { startY = s }
        if let e = engine.nearestSnap(to: endY, in: snapLines.horizontalY) { endY = e }
    }
    ...
    public func sampleColor(atX x: Int, y: Int) {
        guard let target,
              ...
              let (px, w, h) = colorAt(cg: cg, x: x, y: y) else { return }
        let _ = (w, h)
