...
check(GitHubReleaseFetcher.latestURL(owner: "guptaprakhariitr", repo: "caliper").absoluteString
        == "https://api.github.com/repos/guptaprakhariitr/caliper/releases/latest", "builds latest-release URL")
let relJSON = Data(#"""
{"tag_name":"v1.2.0","name":"PicaMac 1.2.0","html_url":"https://github.com/guptaprakhariitr/caliper/releases/tag/v1.2.0","body":"notes","draft":false,"prerelease":false,
 "assets":[{"name":"PicaMac_1.2.0_aarch64.dmg","browser_download_url":"https://github.com/guptaprakhariitr/caliper/releases/download/v1.2.0/PicaMac_1.2.0_aarch64.dmg"}]}
"""#.utf8)
let parsedRel = GitHubReleaseFetcher.parseLatest(from: relJSON)
check(parsedRel?.tagName == "v1.2.0", "parses tag_name")
check(parsedRel?.dmgURL?.lastPathComponent == "PicaMac_1.2.0_aarch64.dmg", "picks .dmg asset")
...
check(parsedRel?.downloadURL.lastPathComponent == "PicaMac_1.2.0_aarch64.dmg", "downloadURL prefers DMG asset")
