...
Draw a precise measurement guide over any UI, magnify pixels with a loupe, and read back colors as hex/HSL. Pica's differentiator is **automatic edge-snap**: a real on-device edge detector finds element boundaries so your measurement endpoints lock to the actual edges, not "close enough." 100% local. Free & open source.

A free, native macOS replacement for paid pixel rulers and color pickers.

![Pica — measuring with auto edge-snap](docs/images/01-measuring.png)

</div>

> The images below are generated by Pica itself (`swift run CaliperChecks docs/images`). To use your own, drop PNGs into `docs/images/` and update the links.
...
Pica runs a Sobel gradient edge detector over the captured pixels, accumulates gradient energy per column and per row, and locks your measurement endpoints to the detected element boundaries — entirely on-device, no network. Toggle it off for free-hand measuring.
...
Pica is an **open-source shell** + a **proprietary engine** (`EdgeEngine`). In public releases the engine ships as a precompiled binary; in this repository it builds from source.

> The app ships as **Pica** (display name + App Store listing). The Swift package, modules, bundle ID (`com.plainware.caliper`) and the on-disk targets below keep the original `Caliper` name, so existing identifiers and Firebase config are unchanged.
...
- **Logging**: every run writes to `~/Library/Containers/com.plainware.caliper/Data/Library/Logs/Plainware/Pica.log` (sandboxed) — `tail -f` it to debug.
